...
fn main() -> ! {
    ...
    loop {
        ...
        let entry1 = FFFF { x: 1.0, y: 2.0 };
        let n =
            bincode::encode_into_slice(&entry1, &mut slice, bincode::config::standard()).unwrap();
        let slice = &slice[..n];
        let res: Result<(FFFF, usize), error::DecodeError> =
        ...
        match res {
            Ok(r) => {
                ...
                delay.delay_ms(1000);
                if r.0.x == entry1.x && r.0.y == entry1.y {
                    led_pin.set_high().unwrap();
                    delay.delay_ms(100);
                    led_pin.set_low().unwrap();
                    delay.delay_ms(100);
...
//         // while let Ok(byte) = uart.read_raw(buffer) {
//         // if byte == 0x02 {
//         // let _ = uart.write_full_blocking("ok".as_bytes());
//         // }
//         // }
//     }
